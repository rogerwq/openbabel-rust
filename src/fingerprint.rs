//! Openbabel Fingerprint
//! 
//! # Environment variable BABEL_DATA setting
//! 
//! PatternFP fingerprint might require data files as input, e.g. the derivatives FP3 and FP4 require data files of patterns.txt and SMARTS_InteLigand.txt respectively.
//! Set BABEL_DATADIR to where those files are located, if "Open Babel Error in Read PatternFile" is encountered,

use openbabel_sys::ob;
use super::molecule;

<<<<<<< HEAD
=======
#[derive(Clone)]
>>>>>>> chiraldb-fp
pub enum Kind {
    FP2 { nbits: u32 },
    FP3 { nbits: u32 },
    FP4 { nbits: u32 },
    ECFP0 { nbits: u32 },
    ECFP2 { nbits: u32 },
    ECFP4 { nbits: u32 },
    ECFP6 { nbits: u32 },
    ECFP8 { nbits: u32 },
    ECFP10 { nbits: u32 }
}

impl Kind {
    fn as_str(&self, thread_id: u32) -> String {
        let fp_name = match self {
            Kind::FP2 { nbits: _ } => "FP2",
            Kind::FP3 { nbits: _ } => "FP3",
            Kind::FP4 { nbits: _ } => "FP4",
            Kind::ECFP0 { nbits: _ } => "ECFP0",
            Kind::ECFP2 { nbits: _ } => "ECFP2",
            Kind::ECFP4 { nbits: _ } => "ECFP4",
            Kind::ECFP6 { nbits: _ } => "ECFP6",
            Kind::ECFP8 { nbits: _ } => "ECFP8",
            Kind::ECFP10 { nbits: _ } => "ECFP10",
        };
        format!("{}_thread_{}", fp_name, thread_id)
    }

    fn get_nbits(&self) -> &u32 {
        match self {
            Kind::FP2 { nbits } => nbits,
            Kind::FP3 { nbits } => nbits,
            Kind::FP4 { nbits } => nbits,
            Kind::ECFP0 { nbits } => nbits,
            Kind::ECFP2 { nbits } => nbits,
            Kind::ECFP4 { nbits } => nbits,
            Kind::ECFP6 { nbits } => nbits,
            Kind::ECFP8 { nbits } => nbits,
            Kind::ECFP10 { nbits } => nbits,
        }
    }
}

struct Fingerprint {
    kind: Kind
}

impl Fingerprint {
    fn new(kind: Kind) -> Self {
        Self { kind }
    }

    fn get_fingerprint(&self, mol: &molecule::Molecule, thread_id: u32) -> cxx::UniquePtr<cxx::CxxVector<u32>> {
        cxx::let_cxx_string!(name = &self.kind.as_str(thread_id));
        ob::OBFingerprint_get_fingerprint(&name, &mol.ob_mol, *self.kind.get_nbits()) // If nbits <=0, nbits = 4096
    }

    fn get_fingerprint_in_batch(&self, smiles_vec: &Vec<String>, thread_id: u32) -> cxx::UniquePtr<cxx::CxxVector<u32>> {
        cxx::let_cxx_string!(name = &self.kind.as_str(thread_id));
        ob::OBFingerprint_get_fingerprint_in_batch(&name, smiles_vec, *self.kind.get_nbits())
    }
}

pub fn get_fingerprint(smiles: &String, kind: &Kind, thread_id: u32) -> cxx::UniquePtr<cxx::CxxVector<u32>> {
    let mol = molecule::Molecule::new_from_smiles(smiles);
    let fp = Fingerprint::new(kind.clone());
    fp.get_fingerprint(&mol, thread_id) 
}

// get fp in batch runs much slower according to benchmark
pub fn get_fingerprint_in_batch(smiles_vec: &Vec<String>, kind: &Kind, thread_id: u32) -> cxx::UniquePtr<cxx::CxxVector<u32>> {
    eprint!("Warning: Getting fingerprint in batch is still under testing");
    let fp = Fingerprint::new(kind.clone());
    fp.get_fingerprint_in_batch(smiles_vec, thread_id)
}

#[cfg(test)]
mod test_mod_fingerprint {
    use super::*;

    #[test]
    #[ignore]
    fn test_error_cases() {
        let thread_id = 0;

        // FP generated by get_fingerprint() and get_fingerprint_in_batch() are different for the same SMILES "CCCN"
        // Root causes to be investigated
        let fpk = Kind::FP2 { nbits: 4096 };
        let fpds = get_fingerprint_in_batch(&vec![String::from("CCCC"), String::from("CCCN")], &fpk, thread_id);
        let fpd = get_fingerprint(&String::from("CCCN"), &fpk, thread_id);
        for i in 0..128 {
            let offset: usize = 128;
            println!("{}: {} - {}", i, fpds.as_ref().unwrap().get(i + offset).unwrap(), fpd.as_ref().unwrap().get(i).unwrap());
            assert_eq!(fpds.as_ref().unwrap().get(i + offset).unwrap(), fpd.as_ref().unwrap().get(i).unwrap());
        }
    }

    #[test]
    fn test_get_fp() {
        let thread_id = 1;
        for fpk in vec![
            Kind::FP2 { nbits: 4096 },
            Kind::FP3 { nbits: 4096 },
            Kind::FP4 { nbits: 4096 },
            Kind::ECFP0 { nbits: 4096 },
            Kind::ECFP2 { nbits: 4096 },
            Kind::ECFP4 { nbits: 4096 },
            Kind::ECFP6 { nbits: 4096 },
            Kind::ECFP8 { nbits: 4096 },
            Kind::ECFP10 { nbits: 4096 },
        ].iter() {
            let fp_data = get_fingerprint(&String::from("CCNCC"), fpk, thread_id);
            assert_eq!(fp_data.len(), 128);
        }
    }

    #[test]
    fn test_get_fp_in_batch() {
        let thread_id = 2;
        for fpk in vec![
            Kind::FP2 { nbits: 4096 },
            Kind::FP3 { nbits: 4096 },
            Kind::FP4 { nbits: 4096 },
            Kind::ECFP0 { nbits: 4096 },
            Kind::ECFP2 { nbits: 4096 },
            Kind::ECFP4 { nbits: 4096 },
            Kind::ECFP6 { nbits: 4096 },
            Kind::ECFP8 { nbits: 4096 },
            Kind::ECFP10 { nbits: 4096 },
        ].iter() {
            let smiles_vec = vec![
                String::from("CCNCC"),
                String::from("c1ccccc1")
            ];
            let fp_data = get_fingerprint_in_batch(&smiles_vec, fpk, thread_id);
            assert_eq!(fp_data.len(), 128 * 2);
        }
    }
}

